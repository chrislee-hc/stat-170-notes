...
In this previous example we could have also written with the ``colloquial'' notation $$ \mathrm d(B_T^2) = 2B_t\mathrm dB_t + \mathrm dt. $$ This is essentially It\^{o}'s formula without the integral signs. More generally, $$ \mathrm d(f(B_t)) = f'(B_t)\mathrm dB_t + \frac{1}{2} f''(B_t)\mathrm dt. $$ 
\ex{Sine Function}{
...
Next suppose we have a function $f(t,x)$ which is dependent on both space and time. Then, $$ \mathrm d(f(t,B_t)) = \partial_t f(t,B_t)\mathrm dt + \partial_x f(t,B_t)\mathrm dB_t + \frac{1}{2}\partial_{xx}^2 f(t,B_t)\mathrm dt. $$ 
